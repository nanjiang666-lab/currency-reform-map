...
// 仅在浏览器端 require mapbox-gl，避免 SSR 报错
...
function Home() {
  ...
  // 构建 Mapbox 的 data-driven fill-color 表达式
  ...
  return (
    <div>
      ...
      {countryCode && (
        <div className="panel">
          <h3>
            {selectedCountry?.name || countryCode} — {year}
          </h3>
          {session?.user ? (
            <>
              <label>
